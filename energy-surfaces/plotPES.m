function PDF = plotPES(annotated_data,var1,var2,verbose)
% Function to plot potential energy surface
addpath(genpath('/lustre/ogunnaike/users/2420/matlab_example/NZ-physiology-data/'))
num_subjects = length(annotated_data);
fig = figure;
fig.Position = [100, 100, 1600, 800];
rows = 2;
missing_data_flag = 0;
t = tiledlayout(fig,rows,round(num_subjects/rows),'TileSpacing','compact','Padding','compact');
for jj = 1:num_subjects
    ...
    if strcmp(var1, 'CO') && strcmp(var2, 'CoBF')
        ...
        if jj == 10
            ...
            X(:,1) = annotated_data(jj).CO(5300000:5330000,1);
            X(:,2) = annotated_data(jj).CoBF(5300000:5330000,1);
...
if verbose == 1
    for jj = 1:length(annotated_data)
        ...
        saveas(gcf,['plot_hist_' num2str(jj) '.png'])
    end
end
