...
t_hours = 1; % Total hours * num_slices shouldn't exceed recording time ~24 hrs
...
for i = 1:length(filesToRead)
    ...
    try
        start(k) = 100+num_points*(k-1);%struct.start;
    catch
        start(k) = 1;%0;
    end
    try
    ...
    stop(k) = 100+num_points*k;%struct.length*interval(i); % points
