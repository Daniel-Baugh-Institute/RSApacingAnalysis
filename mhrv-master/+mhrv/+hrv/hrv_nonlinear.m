function [ hrv_nl, plot_data ] = hrv_nonlinear( nni, filename, varargin )
...
if (should_plot) 
    ...
    hsub1 = subplot(2, 1, 1);
    mhrv.plots.plot_dfa_fn(hsub1, plot_data.dfa);

    hsub3 = subplot(2, 1, 2);
    mhrv.plots.plot_mse(hsub3, plot_data.mse);
    
    try
        exportgraphics(gcf,['./plots_hrv/dfa_mse_' filename])
    catch
        disp('Error: Could not save dfa_mse plot')
    end

    figure;
    hsub2 = subplot(1,1,1);
    mhrv.plots.plot_poincare_ellipse(hsub2, plot_data.poincare);

    try
        exportgraphics(gcf,['./plots_hrv/poincare_' filename])
    catch
        disp('Error: Could not save poincare plot')
    end
end
